...
public abstract class Personnage {
	...
	public Block blockDurDessus(int distanceHaut, Niveau niveau) {
		...
		//Si le personnage se situe dans la map
		if(minY>0) {
			//Si la position du lutin est sur la map
			//(on vérifie ceci en rapportant la position du lutin à la taille de la matrice 
			// et à la taille de notre liste de blocks qui composent la map)
			if((((int) maxX/Block.blockXSize 
				...
				//Si le bloc qui se trouve au dessus du x maximum est un block dur...
				...
				//Si le bloc qui se trouve au dessus du x minimum est un block dur...
			...
			if(((int) maxX/Block.blockXSize 
					...
					* niveau.getMatriceNiveau()[0].length) <= niveau.getBlocksNiveau().size() 
					|| ((int) minX/Block.blockXSize 
							+ (((int) maxY + 10)/Block.blockXSize)
							* niveau.getMatriceNiveau()[0].length) <= niveau.getBlocksNiveau().size()) {
				//Si le bloc qui se trouve au dessous du x maximum est un block dur...
				if(niveau.getBlocksNiveau().get(((int) maxX/Block.blockXSize 
						+ (((int) maxY + 10)/Block.blockXSize)
						* niveau.getMatriceNiveau()[0].length)).isHardBlock()) {
					//On le retourne
					return niveau.getBlocksNiveau().get(((int) maxX/Block.blockXSize 
							+ (((int) maxY + 10)/Block.blockXSize)
							* niveau.getMatriceNiveau()[0].length));
				//Sinon si le bloc qui se trouve au dessous du x minimum est un block dur...
				} else if (niveau.getBlocksNiveau().get(((int) minX/Block.blockXSize 
						+ (((int) maxY + 10)/Block.blockXSize)
						* niveau.getMatriceNiveau()[0].length)).isHardBlock()) {
					//On le retourne
			...
			if((((int) maxX +2)/Block.blockXSize 
					...
					* niveau.getMatriceNiveau()[0].length) <= niveau.getBlocksNiveau().size() 
					|| (((int) maxX+2)/Block.blockXSize 
							...
							* niveau.getMatriceNiveau()[0].length) <= niveau.getBlocksNiveau().size()) {
				//Si le bloc qui se trouve à droite du x maximum et du y maximum est un block dur...
				if(niveau.getBlocksNiveau().get((((int) maxX+2)/Block.blockXSize 
					...
					return niveau.getBlocksNiveau().get((((int) maxX +2)/Block.blockXSize 
							+ (((int) maxY-10)/Block.blockXSize)
							* niveau.getMatriceNiveau()[0].length));
				//Sinon si le bloc qui se trouve à droite du x maximum et du y minimum est un block dur...
	 ...
	 * @param int precision La précision de notre recherche de bloc
	 * @param Niveau niveau Niveau sur lequel on appelle cette fonction
	 * @return Block si dur | Null sinon
	 */
			...
			if(((int) maxX/Block.blockXSize 
					...
					* niveau.getMatriceNiveau()[0].length) <= niveau.getBlocksNiveau().size() 
					|| ((int) minX/Block.blockXSize 
							+ (((int) maxY + 10)/Block.blockXSize)
							* niveau.getMatriceNiveau()[0].length) <= niveau.getBlocksNiveau().size()) {
				//Si le bloc qui se trouve à dessous du x maximum et du y maximum est un block dur...
				if(niveau.getBlocksNiveau().get((((int) maxX-precision)/Block.blockXSize 
					...
					return niveau.getBlocksNiveau().get((((int) maxX-precision)/Block.blockXSize 
							+ (((int) maxY + 10)/Block.blockXSize)
							* niveau.getMatriceNiveau()[0].length));
					//Si le bloc qui se trouve à dessous du x minimum et du y maximum est un block dur...
			...
			if((((int) minX-2)/Block.blockXSize 
					...
					* niveau.getMatriceNiveau()[0].length) <= niveau.getBlocksNiveau().size() 
					|| (((int) minX -2)/Block.blockXSize 
							+ (((int) minY -10)/Block.blockXSize)
							* niveau.getMatriceNiveau()[0].length) <= niveau.getBlocksNiveau().size()) {
				//Si le bloc qui se trouve à gauche du x minimum et du y maximum est un block dur...
				if(niveau.getBlocksNiveau().get((((int) minX-2)/Block.blockXSize 
					...
					return niveau.getBlocksNiveau().get((((int) minX -2)/Block.blockXSize 
							+ (((int) maxY -10)/Block.blockXSize)
							* niveau.getMatriceNiveau()[0].length));
					//Si le bloc qui se trouve à gauche du x minimum et du y minimum est un block dur...
